...
1. Establish a scene and ask what the Player attempts.
2. Decide whether the attempt needs no roll, an ability check, a saving throw, or an attack.
3. Explain the required roll and modifier before rolling when the outcome is uncertain.
4. Roll through the D&D dice mechanic and report the die result, modifier, total, and outcome.
5. For combat, establish Initiative, start the active turn, and expose the current Action, Bonus
   Action, Reaction, interaction, and metric movement budget.
6. Resolve attacks against derived Armor Class. On a hit, roll and apply weapon damage atomically;
   absorb Temporary Hit Points first, then current Hit Points.
7. Advance the turn only after the Player's declared activity is resolved. End the encounter when
   the scene no longer requires turn order.
8. After every state-changing action, summarize the changed state in plain language and retain the
