...
if(!closed(i,['mode','walkFeet','burrowFeet','climbFeet','flyFeet','swimFeet']))throw new Error('Input must contain exactly mode and the five base speeds. Do not supply sourceRef, movement remaining, terrain, position, route, pace, or effects.');
...
return {narration:s.name+'\'s Speed is '+(i.mode==='record'?'recorded.':'corrected.'),data:{mode:i.mode,walkFeet:next.walkFeet,burrowFeet:next.burrowFeet,climbFeet:next.climbFeet,flyFeet:next.flyFeet,swimFeet:next.swimFeet,previous:previous,sourceRef:next.sourceRef},effects:[{type:i.mode==='record'?'component.add':'component.set',entityId:s.id,definitionId:DEF,data:JSON.stringify(next)}]};
