var SID='source.dnd2024.srd-5.2.1',LOC='Rules Glossary',ELOC='Rules Glossary > Exhaustion',DEF='dnd2024.conditions',ORDER=['blinded','charmed','deafened','frightened','grappled','incapacitated','invisible','paralyzed','petrified','poisoned','prone','restrained','stunned','unconscious','exhaustion'],s=ctx.roles.subject,src=ctx.roles.source,i=ctx.input;
...
if(i.mode==='exhaust'||i.mode==='recover'){
  ...
  if(newLevel>0){var exhaustion={condition:'exhaustion',level:newLevel};after.push(exhaustion);added.push(exhaustion);}
  lethal=i.mode==='exhaust'&&newLevel===6;
}else{
...
var next={entries:after,sourceRef:prior.sourceRef},data={mode:i.mode,sourceEntityId:sourceId,beforeEntries:before,afterEntries:after,added:added,removed:removed,removedPoisoned:removedPoisoned,previousLevel:previousLevel,newLevel:newLevel,levelsChanged:(i.mode==='exhaust'||i.mode==='recover')?i.levels:0,lethalEventDeclared:lethal,sourceRef:next.sourceRef};
var result={narration:s.name+' has condition state '+(i.mode==='apply'?'updated.':i.mode==='clear'?'cleared.':i.mode==='exhaust'?'exhausted.':'recovered.'),data:data,effects:[{type:'component.set',entityId:s.id,definitionId:DEF,data:JSON.stringify(next)}]};
if(lethal)result.events=[{type:'dnd2024.exhaustion.reached-lethal',payload:{creatureId:s.id,level:6,sourceRef:{sourceId:SID,locator:ELOC}},entityIds:[s.id]}];
return result;
