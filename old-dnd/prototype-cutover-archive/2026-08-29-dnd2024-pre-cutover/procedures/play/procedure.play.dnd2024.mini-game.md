...
1. At the beginning of a conversation, call `query(kind: "system.audience-context")`. Never use a
   ...
   `characterId` input to `mechanic.dnd2024.character.basic.create` through `procedure.action.run`.
...
- This procedure is chat guidance only. It creates no entities, components, relationships, events,
  notifications, mechanics, public operations, or narrative archive beyond the selected existing
  action's authoritative effects.
