...
3. Browse `query(kind: "categories", catalog: "procedures", category:
   "ruleset.dnd2024.core")` and `query(kind: "categories", catalog: "mechanics", category:
   "ruleset.dnd2024.core")`, then list the relevant branch before searching procedures,
   component definitions or mechanics for an overlap. Revise an existing component rather than
   duplicate it.
...
- Every new D&D procedure or mechanic must use one leaf category path under ruleset.dnd2024.core;
  category queries include that branch and its descendants.
