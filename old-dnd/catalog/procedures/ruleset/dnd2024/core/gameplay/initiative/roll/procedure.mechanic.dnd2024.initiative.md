...
## Description
Defines the D&D 2024 individual Initiative resolver: it reads validated Dexterity and condition
state, applies the established D20 roll-circumstance convention, derives the Initiative count, and
returns a seeded effect-free result.
...
1. Read dnd2024.abilities and dnd2024.conditions on subject. Validate the closed six-score ability shape and integer score range 1 through 30 before randomness. Absent condition state is distinct from known-empty state.
...
3. Validate rollCircumstances exactly as procedure.mechanic.dnd2024.check.ability: unique {kind, source} objects; kind is advantage or disadvantage; source is a nonempty trimmed string. `condition:` is reserved for state-derived evidence and rejected from caller input. Same-kind entries do not stack; any mixture cancels. Surprise is represented only by a validated disadvantage circumstance whose source is "surprised"; it is not persisted or inferred.
