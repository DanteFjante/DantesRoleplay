...
- This v5 revision composes the shared condition state-effects resolver. Caller circumstances stay
  transient; condition circumstances are derived from creature state and reported separately.
...
- rollCircumstances, when present, is an array; absent and [] both mean no caller circumstances. Every member is an object with exactly kind and source. kind is exact lowercase advantage or disadvantage. source is a nonempty already-trimmed string explaining the circumstance. Reject null, wrong type, non-object member, missing/extra member keys, wrong case, unknown kind, blank/untrimmed/non-string source, duplicate exact (kind, source) pairs, and any caller source beginning `condition:`; that prefix is reserved for state-derived evidence.
