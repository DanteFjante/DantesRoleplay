...
public sealed class OllamaAiProviderTests
{
    ...
    [Fact]
    public async Task Generic_provider_lists_models_and_sends_reasoning_with_tools()
    {
        string? body = null;
        ...
        var result = await provider.SendAsync(new(
            "qwen3:8b",
            [new(AiMessageRole.User, "Use a tool if needed.")],
            AiRequestKind.Message,
        ...
        Assert.True(model.Capabilities.HasFlag(AiModelCapabilities.Reasoning));
        Assert.True(model.Capabilities.HasFlag(AiModelCapabilities.Tools));
        Assert.True(result.Ok, result.ErrorMessage);
        using var requestBody = JsonDocument.Parse(body!);
        Assert.True(requestBody.RootElement.GetProperty("think").GetBoolean());
        Assert.Equal("system_read", requestBody.RootElement.GetProperty("tools")[0]
    ...
    public async Task Disabled_provider_never_calls_ollama()
    {
        ...
        var provider = new OllamaAiProvider(new HttpClient(handler), new()
        {
    ...
    public async Task Live_qwen3_8b_returns_schema_valid_no_tools_output_when_enabled()
    {
        ...
        var provider = new OllamaAiProvider(new HttpClient(), new()
        {
            Enabled = true,
            Model = "qwen3:8b",
    ...
    private static OllamaAiProvider Provider(AsyncHandler handler) =>
