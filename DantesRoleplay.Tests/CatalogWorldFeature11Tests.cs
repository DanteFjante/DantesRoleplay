...
public sealed class CatalogWorldFeature11Tests : IDisposable
{
    ...
    private static Task<ActionRunResult> AdvanceAsync(CatalogMechanicTestHarness runner, string expectedPhase) => runner.RunAsync(new ActionRequest { Intent = "advance the observatory front", RoleEntityIds = Roles(), Input = $"{{\"expectedPhase\":\"{expectedPhase}\"}}", Seed = 1111 });
