...
public sealed class SubscriptionStore(DantesRoleplayDbContext db) : ISubscriptionStore
{
    ...
    public async Task<WriteSubscriptionResult> WriteAsync(WriteSubscriptionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var fixedRoles = SubscriptionCanonicalJson.Object(request.FixedRoleEntityIdsJson); var payloadRole = SubscriptionCanonicalJson.Object(request.RoleFromEventPayloadJson); var fanout = SubscriptionCanonicalJson.Object(request.FanoutSelectorJson); var tracked = SubscriptionCanonicalJson.Ids(request.TrackedEntityIdsJson); var payload = SubscriptionCanonicalJson.Object(request.PayloadEqualsJson); var now = DateTime.UtcNow;
