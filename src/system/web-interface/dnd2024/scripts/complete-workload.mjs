...
export function completeWorkloadEvidence(source) {
  ...
  for (const group of groups) {
    ...
    if (!baseline || !count(baseline.sampleCount) || baseline.sampleCount < 20 || !finite(baseline.loaderP50Ms))
      missing.add(`Missing matched ${group} baseline loader timing.`);
    else if (samples.length >= 20 && samples.every(run => finite(run.marks?.completeWorkload)) &&
        median(samples.map(run => run.marks.completeWorkload)) > baseline.loaderP50Ms * 0.5)
