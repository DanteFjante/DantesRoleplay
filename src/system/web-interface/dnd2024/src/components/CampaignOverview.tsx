...
export function CampaignOverview({
  ...
  const logUnavailable = campaign.detailFields
    ...
    <div className="campaign-overview">
      ...
      {detailsAvailable ? <section className="campaign-overview-grid campaign-overview-grid--lower">
        <article className="panel">
            <div className="panel-heading">
            <div><span className="eyebrow">Latest memory</span><h2>{latest?.title ?? (logUnavailable ? "Campaign memories unavailable" : "No completed entries yet")}</h2></div>
          ...
          {latest
            ...
            : <p className="campaign-summary-copy">{logUnavailable
              ? "Some campaign memories are unavailable for this read; omitted entries are not confirmed absent."
              : "Completed chapters and retained session recaps will appear here when the live campaign records them."}</p>}
        </article>
        <article className="panel">
            <div className="panel-heading">
            <div><span className="eyebrow">Campaign trail</span><h2>{visitsUnavailable ? "Campaign trail unavailable" : `${campaign.placesVisited.length} places remembered`}</h2></div>
            <Icon className="heading-icon" name="MapPin" />
          </div>
          <p className="campaign-summary-copy">{visitsUnavailable
      ...
      {detailsAvailable ? <section className="campaign-overview-grid campaign-overview-grid--lower">
        ...
        <article className="panel">
          <div className="panel-heading">
            <div><span className="eyebrow">Party knowledge</span><h2>{campaign.cluesCoverage === "unavailable"
              ? "Clues not loaded" : `${campaign.clues.length} recorded clues${campaign.cluesCoverage === "partial" ? " · partial view" : ""}`}</h2></div>
            <Icon className="heading-icon" name="Search" />
          </div>
          <p className="campaign-summary-copy">{campaign.cluesCoverage === "unavailable"
            ? "Open Clues to load the authorized evidence. Unloaded clues are not confirmed absent."
            : campaign.cluesCoverage === "partial"
            ? "Some clue records are unavailable in this read. Omitted clues are not confirmed absent."
            : campaign.clues.length
