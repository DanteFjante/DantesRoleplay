...
export function CampaignClues({
  ...
  return <div className="campaign-section-view">
    ...
    {clues.length ? <section aria-label="Campaign clues" className="campaign-clue-grid">{clues.map((clue) => <article className="campaign-clue-card" data-record-id={clue.id} key={clue.id}><header><div><small>{clue.mystery} · {clue.discoveredAt}</small><h2>{clue.title}</h2></div><span>{clue.status}</span></header>{clue.handout ? <figure className="campaign-clue-card__handout"><MediaImage fallback={null} media={clue.handout} /></figure> : null}<p>{clue.detail}</p><div className="campaign-party-conclusion"><small>The party's conclusion</small><p>{clue.partyConclusion}</p></div><CampaignEntityLinks links={clue.links} onOpenFaction={onOpenFaction} onOpenLocation={onOpenLocation} onOpenPerson={onOpenPerson} />{clue.dmTruth || clue.dmConnection ? <aside className="campaign-dm-context"><span>DM clue context</span>{clue.dmTruth ? <p><strong>Truth</strong>{clue.dmTruth}</p> : null}{clue.dmConnection ? <p><strong>Connection</strong>{clue.dmConnection}</p> : null}</aside> : null}</article>)}</section> : <CampaignEmptyState description={campaign.clues.length ? "Try a broader mystery, status, or search." : "The live game has not recorded campaign-owned clue entries yet. World knowledge remains available in the World tab."} icon="Search" title={campaign.clues.length ? "No clues match" : "No campaign clues recorded yet"} />}
  </div>;
}
