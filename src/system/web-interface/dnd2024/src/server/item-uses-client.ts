import { contract } from "./item-uses-validator.js";
...
import { projectItemUses } from "./item-display-projection";
...
export type UseEntry = {
  ...
  kind: "canonical-activity" | "recorded-application" | "unknown";
...
export async function readItemUses(request: ItemUsesRequest, signal: AbortSignal, fetchImpl: typeof fetch = fetch): Promise<ItemUsesResult> {
  const input = { itemId: request.itemId, offset: request.offset, expectedSourceRevision: request.expectedSourceRevision,
    selectionId: request.campaignId };
  ...
  return readItemResponse<ItemUsesData>({ request, input, contract, expectedSourceRevision: input.expectedSourceRevision,
    consume: (value) => {
      const uses = projectItemUses(value, request, request.offset);
      return uses ? { version: 1, observerId: request.observerId, itemId: request.itemId, perspective: request.perspective, uses } : null;
    },
    errorMessage: "Use response did not match the selected view.", verify: (data) => {
      ...
      return data.itemId === request.itemId && data.observerId === request.observerId && data.perspective === request.perspective &&
        ...
        (group.nextOffset === null || group.nextOffset > request.offset && group.nextOffset <= 10000);
    },
  }, signal, fetchImpl);
}
