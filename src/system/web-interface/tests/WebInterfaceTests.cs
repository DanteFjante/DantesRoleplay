...
public sealed class WebInterfaceTests
{
    ...
    public async Task Application_surface_is_exact_and_components_have_no_control_authority()
    {
        ...
        Assert.Contains("class Dnd2024CharacterSheet extends HTMLElement", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("customElements.define('dnd2024-character-sheet'", dndScript,
            StringComparison.Ordinal);
        ...
        Assert.Contains("button.setAttribute('role', 'tab')", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("panel.setAttribute('role', 'tabpanel')", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("_handleViewTabKey(event, key)", dndScript, StringComparison.Ordinal);
        Assert.Contains("event.key === 'ArrowRight'", dndScript, StringComparison.Ordinal);
        Assert.Contains("event.key === 'ArrowLeft'", dndScript, StringComparison.Ordinal);
        Assert.Contains("event.key === 'Home'", dndScript, StringComparison.Ordinal);
        ...
        Assert.Contains("campaignView.append(this._campaignPanel.panel)", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("combatView.append(this._encounter.panel, this._turn.panel)", dndScript,
            StringComparison.Ordinal);
        ...
        Assert.Contains("DND2024_CHARACTER_COMPONENTS = Object.freeze", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("async _findCampaigns", dndScript, StringComparison.Ordinal);
        Assert.Contains("DND2024_CAMPAIGN_ROOT_COMPONENT", dndScript, StringComparison.Ordinal);
        Assert.Contains("DND2024_CAMPAIGN_PARTICIPATION_KINDS", dndScript, StringComparison.Ordinal);
        Assert.Contains("DND2024_PARTICIPATION_ACTOR_KINDS", dndScript, StringComparison.Ordinal);
        Assert.Contains("dnd2024.game.core.campaign.has-character-participation", dndScript, StringComparison.Ordinal);
        Assert.Contains("DND2024_LEGACY_CHARACTER_COMPONENT", dndScript, StringComparison.Ordinal);
        Assert.Contains("async _readRelationships", dndScript, StringComparison.Ordinal);
        Assert.Contains("/relationships", dndScript, StringComparison.Ordinal);
        ...
        Assert.Contains("No registered D&D campaign space is available.", dndScript,
            StringComparison.Ordinal);
        ...
        Assert.Contains("No registered campaign root exists in this campaign space.", dndScript,
            StringComparison.Ordinal);
        ...
        Assert.Contains("bubbles: true, composed: true", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("characterProfile: 'dnd2024.character.identity'", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("characterExperience: 'dnd2024.character.experience'", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("hitPoints: 'dnd2024.creature.hit-points'", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("temporaryHitPoints: 'dnd2024.creature.temporary-hit-points'", dndScript,
            StringComparison.Ordinal);
        ...
        Assert.Contains("'encounter-id'", dndScript, StringComparison.Ordinal);
        Assert.Contains("async _findRecordedEncounters", dndScript, StringComparison.Ordinal);
        Assert.Contains("this._entityComponentSummaries = new Map()", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("this._entityComponentSummaries.get(candidate.entityId) ||", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("candidateParticipantIds.every(id => campaignActorIds.has(id))", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("_encounterRosterMatches(order, rosterPage", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("customElements.define('dnd2024-encounter-tracker'", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("customElements.define('dnd2024-turn-budget'", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.encounter-turn.start", dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.encounter-turn.advance", dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.encounter-turn.end", dndScript, StringComparison.Ordinal);
        Assert.Contains("control.roleEntityIds = {encounter: this.encounterEntityId}", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.turn-budget.spend", dndScript, StringComparison.Ordinal);
        Assert.Contains("control.roleEntityIds = {subject: this.subjectEntityId, encounter: this.encounterEntityId}",
            dndScript, StringComparison.Ordinal);
        Assert.Contains("input = {resource: 'movement', feet: this._movementFeet}", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("dnd2024-encounter-refresh", dndScript, StringComparison.Ordinal);
        Assert.Contains("Spend 5-foot movement increments", dndScript, StringComparison.Ordinal);
        ...
        Assert.Contains("DND2024_SCENE_MAXIMUM_ENTRIES = 24", dndScript, StringComparison.Ordinal);
        Assert.Contains("DND2024_WORLD_LOCATION_COMPONENT = 'dnd2024.game.core.world.location'",
            dndScript, StringComparison.Ordinal);
        Assert.Contains("DND2024_WORLD_MOTIVE_COMPONENT = 'dnd2024.game.core.world.motive'",
            dndScript, StringComparison.Ordinal);
        ...
        Assert.Contains("async _loadCurrentContext(stateSpaceId, entityId, signal)", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("entityRoot + '/containment'", dndScript, StringComparison.Ordinal);
        Assert.Contains("containment.slot !== 'presence'", dndScript, StringComparison.Ordinal);
        Assert.Contains("row.containerEntityId !== locationId", dndScript, StringComparison.Ordinal);
        Assert.Contains("stateSpaceId, locationId, DND2024_SCENE_MAXIMUM_ENTRIES, signal", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("async _loadScenePerson(stateSpaceId, entityId, signal)", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("_renderLocation()", dndScript, StringComparison.Ordinal);
        Assert.Contains("_renderScenePeople()", dndScript, StringComparison.Ordinal);
        Assert.Contains("Current location is not recorded for this character.", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("This scene view shows the first 24 direct contents.", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("Motive text is not exposed as player knowledge.", dndScript,
            StringComparison.Ordinal);
        ...
        Assert.Contains("Bounded nested contents · contextual actions", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("Ordinary actions appear only where their complete required state is visible", dndScript, StringComparison.Ordinal);
        Assert.Contains("_appendEquipmentActions(card, entry, instance, definition, quantity, equipment",
            dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.item.equip", dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.item.unequip", dndScript, StringComparison.Ordinal);
        Assert.Contains("depth !== 1", dndScript, StringComparison.Ordinal);
        Assert.Contains("containment.containerEntityId !== this.selectedEntityId", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("quantity.status !== 'missing'", dndScript, StringComparison.Ordinal);
        Assert.Contains("definition.value.equipmentModes", dndScript, StringComparison.Ordinal);
        Assert.Contains("new Set(declaredModes).size === declaredModes.length", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("{item: configuration.itemId, holder: this.selectedEntityId}",
            dndScript, StringComparison.Ordinal);
        Assert.Contains("control.roleEntityIds = configuration.roles ||", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("itemId, input: {state: selectedMode}", dndScript, StringComparison.Ordinal);
        Assert.Contains("label: 'Prepare unequip', itemId, input: {}", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("Prepare hold", dndScript, StringComparison.Ordinal);
        Assert.Contains("Prepare wear", dndScript, StringComparison.Ordinal);
        Assert.Contains("itemActivity: 'dnd2024.item-activity'", dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.item.transfer", dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.item-stack.consume", dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.item-stack.split", dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.item-stack.merge", dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.item-activity.use", dndScript, StringComparison.Ordinal);
        Assert.Contains("roles: {item: itemId, source: sourceId, destination: selected.id}", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("roles: {item: itemId, definition: stack.definitionId}", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("roles: {source: itemId, definition: stack.definitionId}", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("roles: {source: itemId, target: state.mergeTargetId, definition: stack.definitionId}",
            dndScript, StringComparison.Ordinal);
        Assert.Contains("grantDefinition: activity.grant.definitionId", dndScript, StringComparison.Ordinal);
        Assert.Contains("input: {activityId: activity.id, grantItemId}", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("item.web.${kind}.${token}", dndScript, StringComparison.Ordinal);
        Assert.Contains("Consuming the full displayed count may remove this stack.", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("The current stack is the merge source and may be removed.", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("The server checks ordinary admission, capacity, and cycles.", dndScript,
            StringComparison.Ordinal);
        ...
        Assert.Contains("_renderVitalActions(body, hp, temp)", dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.healing.apply", dndScript, StringComparison.Ordinal);
        Assert.Contains("mechanic.dnd2024.temporary-hit-points.write", dndScript, StringComparison.Ordinal);
        Assert.Contains("input: {amount: this._healingAmount}", dndScript, StringComparison.Ordinal);
        Assert.Contains("const temporaryInput = {mode: 'grant', amount: this._temporaryHitPointsAmount}", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("temporaryInput.onExisting = this._temporaryHitPointsChoice", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("input: {mode: 'expire'}", dndScript, StringComparison.Ordinal);
        Assert.Contains("Keep current", dndScript, StringComparison.Ordinal);
        Assert.Contains("Replace with incoming", dndScript, StringComparison.Ordinal);
        Assert.Contains("roleEntityIds = {subject: this.selectedEntityId}", dndScript,
            StringComparison.Ordinal);
        Assert.Contains("application-receipt", dndScript, StringComparison.Ordinal);
        Assert.Contains("event.detail?.phase === 'complete'", dndScript, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("mechanic.dnd2024.hit-points.write", dndScript, StringComparison.Ordinal);
        Assert.DoesNotContain("mechanic.dnd2024.item-instance.move", dndScript, StringComparison.Ordinal);
        Assert.DoesNotContain("mechanic.dnd2024.item-instance.create-and-place", dndScript,
            StringComparison.Ordinal);
        Assert.DoesNotContain("mechanic.dnd2024.item-stack.create-and-place", dndScript,
            StringComparison.Ordinal);
        Assert.DoesNotContain("mechanic.dnd2024.item-instance.record", dndScript,
            StringComparison.Ordinal);
        Assert.DoesNotContain("mechanic.dnd2024.item-stack.record", dndScript,
            StringComparison.Ordinal);
        Assert.DoesNotContain("mechanic.dnd2024.encounter-initiative-order", dndScript,
            StringComparison.Ordinal);
        Assert.DoesNotContain("mechanic.dnd2024.weapon-attack", dndScript,
            StringComparison.Ordinal);
        Assert.DoesNotContain("mechanic.dnd2024.weapon-damage", dndScript,
            StringComparison.Ordinal);
        Assert.DoesNotContain("component.set", dndScript, StringComparison.Ordinal);
        Assert.DoesNotContain("component.add", dndScript, StringComparison.Ordinal);
        Assert.DoesNotContain("component.remove", dndScript, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("dnd2024-scene", dndScript, StringComparison.Ordinal);
        Assert.DoesNotContain("dnd2024-knowledge", dndScript, StringComparison.Ordinal);
        Assert.DoesNotContain("dnd2024-map", dndScript, StringComparison.Ordinal);
