...
public sealed partial class SqliteStandingGrantTargetResolverTests
{
    ...
    public async Task Candidate_validation_retains_unavailable_pipeline_diagnostics_and_inspection_exposes_them()
    {
        ...
        _ = await service.WriteCandidateAsync(ApplicationHost(setup, "validation-write", InteractionExecutionProfile.Atomic), Request(setup.Activation.Current(Application)!.ActivationFingerprint));
        ...
        var validation = await service.ValidateAsync(ApplicationHost(setup, "validation", InteractionExecutionProfile.Atomic), candidate);
        var inspected = await service.InspectAsync(ApplicationHost(setup, "validation-inspect"), new(row.CandidateId, row.Revision));
    ...
    public async Task Candidate_validation_replay_reuses_its_receipt_without_another_validation_ledger()
    {
        ...
        var service = Service(db, setup);
        _ = await service.WriteCandidateAsync(ApplicationHost(setup, "validation-replay-write", InteractionExecutionProfile.Atomic), Request(setup.Activation.Current(Application)!.ActivationFingerprint));
        var row = Assert.Single(await db.Set<ApplicationCandidateRevisionRecord>().ToArrayAsync());
        var candidate = new ApplicationCandidateReference(Application, row.CandidateId, row.Revision, row.ContentFingerprint);

        var first = await service.ValidateAsync(ApplicationHost(setup, "validation-replay", InteractionExecutionProfile.Atomic), candidate);
        ...
        var replay = await service.ValidateAsync(ApplicationHost(setup, "validation-replay", InteractionExecutionProfile.Atomic), candidate);
    ...
    public async Task Candidate_validation_requires_current_validate_authority_and_exact_candidate_pin()
    {
        ...
        var service = Service(db, setup);
        _ = await service.WriteCandidateAsync(ApplicationHost(setup, "validation-denied-write", InteractionExecutionProfile.Atomic), Request(setup.Activation.Current(Application)!.ActivationFingerprint));
        var row = Assert.Single(await db.Set<ApplicationCandidateRevisionRecord>().ToArrayAsync());
        var candidate = new ApplicationCandidateReference(Application, row.CandidateId, row.Revision, row.ContentFingerprint);

        var denied = await service.ValidateAsync(ApplicationHost(setup, "validation-denied", InteractionExecutionProfile.Atomic), candidate);
        var tampered = await service.ValidateAsync(ApplicationHost(setup, "validation-tampered", InteractionExecutionProfile.Atomic), candidate with { ContentFingerprint = new string('A', 64) });
    ...
    public async Task Revoked_validate_grant_denies_validation_replay_without_a_second_ledger()
    {
        ...
        await SeedGrantAsync(db, [StandingGrantCapability.Author, StandingGrantCapability.Validate]);
        var service = Service(db, setup);
        _ = await service.WriteCandidateAsync(ApplicationHost(setup, "validation-revoked-write", InteractionExecutionProfile.Atomic), Request(setup.Activation.Current(Application)!.ActivationFingerprint));
        var row = Assert.Single(await db.Set<ApplicationCandidateRevisionRecord>().ToArrayAsync());
        var candidate = new ApplicationCandidateReference(Application, row.CandidateId, row.Revision, row.ContentFingerprint);
        _ = await service.ValidateAsync(ApplicationHost(setup, "validation-revoked", InteractionExecutionProfile.Atomic), candidate);
        await RevokeGrantAsync(db);

        var replay = await service.ValidateAsync(ApplicationHost(setup, "validation-revoked", InteractionExecutionProfile.Atomic), candidate);
    ...
    public async Task Tampered_validation_diagnostics_cannot_replay_a_prior_receipt()
    {
        ...
        await SeedGrantAsync(db, [StandingGrantCapability.Author, StandingGrantCapability.Validate]);
        var service = Service(db, setup);
        _ = await service.WriteCandidateAsync(ApplicationHost(setup, "validation-tamper-write", InteractionExecutionProfile.Atomic), Request(setup.Activation.Current(Application)!.ActivationFingerprint));
        ...
        _ = await service.ValidateAsync(ApplicationHost(setup, "validation-tamper", InteractionExecutionProfile.Atomic), candidate);
        var validation = await db.Set<ApplicationCandidateValidationRecord>().SingleAsync();
        validation.DiagnosticsJson = "[]";
        await db.SaveChangesAsync();

        var replay = await service.ValidateAsync(ApplicationHost(setup, "validation-tamper", InteractionExecutionProfile.Atomic), candidate);
