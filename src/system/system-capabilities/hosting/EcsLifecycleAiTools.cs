...
public sealed class EcsLifecycleAiToolSource(IEcsLifecycleStore lifecycle) : ISystemAiToolSource
{
        ...
        IAiToolApprovalGate? approval) : Tool
    {
        public override AiToolDefinition Definition { get; } = new(
            ...
            "Correct an unused component-type ID, migrate live components into an existing canonical type, upgrade older live values to the latest immutable version by supplying the same source and target ID, enable or disable a type, or permanently delete a disabled unreferenced type. Migration validates every value against the target schema and can supply exact revision-bound replacement values. Immutable projection or trigger definitions block identity changes. Trusted host approval is required.",
