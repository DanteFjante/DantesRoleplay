...
public sealed class ApplicationMechanicExecutionTests : IDisposable
{
    ...
    {
        var app = ApplicationIdentifier.Parse("fixture");
        var childContent = JsonSerializer.Serialize(new { requirements = "{}", source = "return {data:{}};" });
        var child = new CatalogRecordDefinition(app.Value, "mechanic", "fixture.mechanic.child",
            "Read projection child", "Read projection child.", [], [], "mechanics", "active", 1, childContent,
            Hash(childContent), "catalog", "mechanics/child.md");
        ...
        var parent = new CatalogRecordDefinition(app.Value, "mechanic", "fixture.mechanic.read",
            "Read projection", "Read projection.", [], [], "mechanics", "active", 1, parentContent,
            Hash(parentContent), "catalog", "mechanics/read.md");
        ...
        var action = await evaluator.EvaluateAsync(new("space", app, parent.QualifiedId,
            parent.ContentFingerprint, mapping, new Dictionary<string, string>(), "{}", 1));
        var readModel = await evaluator.EvaluateAsync(new("space", app, parent.QualifiedId,
            parent.ContentFingerprint, mapping, new Dictionary<string, string>(), "{}", 2,
            ReadModelQueryId: "fixture.query.read"));

        Assert.True(action.Ok);
        Assert.True(readModel.Ok);
        Assert.Equal(4, engine.Limits.Count);
        Assert.All(engine.Limits.Take(2), limits => Assert.Equal(ExecutionLimits.Default, limits));
