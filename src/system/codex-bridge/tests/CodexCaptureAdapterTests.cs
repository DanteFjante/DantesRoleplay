...
public sealed class CodexCaptureAdapterTests
{
    ...
    public async Task Reads_only_the_requested_completed_turn_with_real_visible_messages()
    {
        ...
        Assert.Equal(["user", "assistant"], turn.Messages.Select(message => message.Role));
        Assert.Equal(["User words", "Visible answer"], turn.Messages.Select(message => message.Content));
    ...
    public async Task Excludes_reasoning_hidden_and_tool_items()
    {
        var client = new FakeReadClient(Json("""
            {"thread":{"id":"thread.gameplay","turns":[{"id":"turn.1","status":"completed","items":[
              {"id":"u1","type":"userMessage","text":"real user","visibility":"visible"},
        ...
        Assert.Equal(["real user", "visible assistant"], turn!.Messages.Select(message => message.Content));
    }
    ...
    private static CodexCaptureAdapter Adapter(FakeReadClient client, CodexCaptureCorrelationSpool spool) => new(client,
        new(Repository, "project.caldris", "session.gameplay", "thread.gameplay"), spool);
    private static CodexCaptureHookInput Hook(string eventName, string turn, string prompt,
        string session = "session.gameplay", string cwd = Repository, string transcript = "C:\\ignored.jsonl") =>
            ...
            thread = new
            {
                ...
                turns = new[]
                {
                    new
                    {
                        ...
                        items = new object[]
                        {
                            new { id = "user.1", type = "userMessage", text = user, visibility = "visible" },
