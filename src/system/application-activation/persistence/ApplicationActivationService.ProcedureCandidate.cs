...
{
    internal async Task<ActiveApplicationManifest> StageReviewedProcedureUpdateAsync(
        ApplicationCandidateReviewedProcedureUpdateEvidence update,
        ...
        CancellationToken cancellationToken)
    {
        if (db.Database.CurrentTransaction is null || validation.Outcome != "valid"
            ...
            || !ApplicationCandidateReviewedProcedureUpdateValidation.Matches(validation, update))
            throw Invalid("APPLICATION_CANDIDATE_PUBLICATION_INVALID",
                "A validated reviewed procedure candidate and the owning transaction are required.");
        ...
        var fingerprint = InteractionCanonicalJson.Fingerprint(
            "dantes-roleplay/reviewed-procedure-candidate-activation/v1",
