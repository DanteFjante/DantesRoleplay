...
{
    /// <summary>Stages a structurally proved Matches-only generation in the owning transaction.</summary>
    internal async Task<ActiveApplicationManifest> StageIntentMatchUpdateAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (db.Database.CurrentTransaction is null || validation.Outcome != "valid"
            ...
            throw Invalid("APPLICATION_CANDIDATE_PUBLICATION_INVALID",
                "A validated intent match candidate and the owning transaction are required.");
        ...
        var fingerprint = InteractionCanonicalJson.Fingerprint(
            "dantes-roleplay/intent-match-activation/v1",
            InteractionCanonicalJson.CanonicalizeObject(System.Text.Json.JsonSerializer.Serialize(new
            {
                update.Candidate,
                basis = update.Basis.ActivationFingerprint,
                update.Fingerprint,
                validation.PreparationVersion,
                validation.PreparedEvidenceReference,
        ...
        db.Add(new ApplicationCandidatePublicationRecord
        {
            ...
            Revision = update.Candidate.Revision
        });
