...
/// <summary>
/// Test-only harness for exercising retained catalog mechanics against the quarantined legacy
/// stores while their live records await explicit adoption. It is not registered by production.
/// </summary>
public sealed class CatalogMechanicTestHarness(
    ...
    IMechanicComposer? composer = null)
{
    ...
    private static ActionRunError? ValidateRequest(ActionRequest request)
    {
        ...
        if (!MechanicInput.TryValidateObject(request.Input, out var inputProblem))
        {
            return new ActionRunError(
                "INVALID_INPUT",
                inputProblem!,
