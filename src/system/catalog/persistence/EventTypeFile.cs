...
public sealed record EventTypeFile(string Id, string Category, string Name, string Description, string Scope, EventTypeStatus Status, string Schema, string CreatedBy = "", string ChangeNote = "")
{
    ...
    public string ToJson() => JsonSerializer.Serialize(new Payload(Id, Category, Name, Description, Scope, Status.ToString(), CreatedBy, ChangeNote), Json) + "\n";
    public static EventTypeFile Parse(string json, string source, string? schema)
    {
        ...
        return new(p.Id.Trim(), p.Category.Trim(), p.Name.Trim(), p.Description ?? string.Empty, p.Scope ?? string.Empty, status, schema.Trim(), p.CreatedBy ?? string.Empty, p.ChangeNote ?? string.Empty);
    }
    private sealed record Payload(string? Id, string? Category, string? Name, string? Description, string? Scope, string? Status, string? CreatedBy = null, string? ChangeNote = null);
}
