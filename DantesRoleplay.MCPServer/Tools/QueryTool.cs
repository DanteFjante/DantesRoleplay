...
public sealed class QueryTool
{
        ...
        IEntityComponentStore? applicationEntities = null)
    {
        ...
        return normalizedKind switch
        {
            ...
            "system.catalog.browse" => await new SystemCatalogTools().BrowseAsync(
                publicCatalogs ?? new EmptyPublicApplicationCatalogProvider(), log, applicationId, collection, branch, pageSize, cursor),
            "system.catalog.search" => await new SystemCatalogTools().SearchAsync(
            ...
            "system.catalog.record" => await new SystemCatalogTools().RecordAsync(
                publicCatalogs ?? new EmptyPublicApplicationCatalogProvider(), log, applicationId, collection, id),
            "system.feature-search" => await new SystemInteractionTools().SearchAsync(
                interactionGateway, privateOperator, log, applicationId, query, id, limit, cancellationToken),
            "system.interaction-plan" => await new SystemInteractionTools().PlanAsync(
                interactionGateway, privateOperator, log, applicationId, request, cancellationToken),
            "system.interaction-receipt" => await new SystemInteractionTools().ReceiptAsync(
