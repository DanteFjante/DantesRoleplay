...
public sealed class EventTypeHandler
{
    public async Task<ToolEnvelope> FindAsync(IEventTypeStore store, IOperationLog log, string? id, int? version, string? query, string? category, string? scope, bool includeInactive, int? limit, CancellationToken cancellationToken) =>
        await ToolRunner.RunAsync(log, "find_event_types", async () =>
        {
            if (!string.IsNullOrWhiteSpace(id))
            {
                ...
                if (item is not null) return ToolOutcome.OkAbout(id, item, $"Read event type {id} v{item.Version}.", $"{McpVerbCatalog.CommitCall("event-type", id, true)} — dry run a revision first.");
            ...
            return ToolOutcome.Ok(new { EventTypes = items }, $"Found {items.Count} event type(s).", items.Count == 0 ? McpVerbCatalog.CommitCall("event-type", true) : $"query(kind: \"event-types\", id: \"{items[0].Id}\")");
        });
    ...
    public async Task<ToolEnvelope> WriteAsync(IEventTypeStore store, IOperationLog log, string id, string category, string name, string schema, string description, string scope, string? status, string changeNote, string intent, string[]? proceduresUsed, bool dryRun, CancellationToken cancellationToken) =>
        await ToolRunner.RunAsync(log, "write_event_type", intent, id, proceduresUsed, async () =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(status) && !Enum.TryParse<EventTypeStatus>(status, true, out _)) return ToolOutcome.Fail("INVALID_STATUS", $"'{status}' is not a status.", McpVerbCatalog.CommitCall("event-type", id), $"Rejected '{id}': bad status.");
            ...
            if (dryRun) return ToolOutcome.Ok(new { Checks = checks, CanWrite = checks.All(x => x.Passed || !x.Blocking) }, $"Dry run for event type '{id}'.", McpVerbCatalog.CommitCall("event-type", id));
            var failed = checks.FirstOrDefault(x => x.Blocking && !x.Passed); if (failed is not null) return ToolOutcome.Fail("INVALID_EVENT_TYPE", failed.Detail, McpVerbCatalog.CommitCall("event-type", id, true), $"Rejected event type '{id}'.");
