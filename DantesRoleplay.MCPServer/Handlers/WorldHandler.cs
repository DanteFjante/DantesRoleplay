        ...
        await ToolRunner.RunAsync(log, "describe_world", async () =>
        {
            ...
            if (definitions.Count == 0)
            {
                nextSteps.Add(
                    ...
                    $"{McpVerbCatalog.CommitCall("component")} declares the first one.");
            }
            else
            {
                nextSteps.Add($"query(kind: \"entities\", withDefinitionId: \"{definitions[0].Id}\") — see what carries that component.");
                nextSteps.Add($"{McpVerbCatalog.CommitCall("effects", dryRun: true)} — validate a change before making it.");
            }
        ...
        await ToolRunner.RunAsync(log, "get_entities", async () =>
        {
            if (ids is { Length: > 0 })
            {
                ...
                return ToolOutcome.Ok(
                    ...
                    $"{McpVerbCatalog.CommitCall("effects", dryRun: true)} — validate a change against what you just read.");
            }
            ...
            if (results.Count == 0)
            {
                ...
                return ToolOutcome.Ok(
                    ...
                    any.Count > 0
                        ...
                        : $"The world is empty. {McpVerbCatalog.CommitCall("effects", dryRun: true)} — an entity.create effect makes the first one.");
            }
        ...
        await ToolRunner.RunAsync(log, "define_component", intent, id, proceduresUsed, async () =>
        {
            if (string.IsNullOrWhiteSpace(id) || string.IsNullOrWhiteSpace(name))
            {
                return ToolOutcome.Fail(
                    ...
                    McpVerbCatalog.CommitCall("component"),
        ...
        await ToolRunner.RunAsync(log, "apply_effects", intent, string.Empty, proceduresUsed, async () =>
        {
            ...
            if (effects.Length == 0)
            {
                return ToolOutcome.Fail(
                    ...
                    McpVerbCatalog.CommitCall("effects", dryRun: true),
            ...
            if (!result.Valid)
            {
                return ToolOutcome.Fail(
                    ...
                    $"{McpVerbCatalog.CommitCall("effects", dryRun: true)} — correct the effects "
