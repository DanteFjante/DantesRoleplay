...
public sealed class SubscriptionHandler
{
    public async Task<ToolEnvelope> FindAsync(ISubscriptionStore store, IOperationLog log, string? id, int? version, string? query, string? category, string? scope, bool includeInactive, int? limit, CancellationToken cancellationToken) =>
        await ToolRunner.RunAsync(log, "find_subscriptions", async () =>
        {
            ...
            return ToolOutcome.Ok(new { Subscriptions = items }, $"Found {items.Count} subscription(s).", items.Count == 0 ? McpVerbCatalog.CommitCall("subscription", true) : $"query(kind: \"subscriptions\", id: \"{items[0].Id}\")");
        });
    ...
    public async Task<ToolEnvelope> WriteAsync(ISubscriptionStore store, IOperationLog log, WriteSubscriptionRequest request, string intent, string[]? proceduresUsed, bool dryRun, CancellationToken cancellationToken) =>
        await ToolRunner.RunAsync(log, "write_subscription", intent, request.Id, proceduresUsed, async () =>
        {
            ...
            if (failed?.Name == "mode-immutable") return ToolOutcome.Fail("MODE_IMMUTABLE", failed.Detail, McpVerbCatalog.CommitCall("subscription", dryRun: true), $"Rejected mode revision for '{request.Id}'.");
            if (dryRun) return ToolOutcome.Ok(new { Checks = checks, CanWrite = failed is null }, $"Dry run for subscription '{request.Id}'.", McpVerbCatalog.CommitCall("subscription", request.Id));
            if (failed is not null) return ToolOutcome.Fail("INVALID_SUBSCRIPTION", failed.Detail, McpVerbCatalog.CommitCall("subscription", request.Id, true), $"Rejected subscription '{request.Id}'.");
