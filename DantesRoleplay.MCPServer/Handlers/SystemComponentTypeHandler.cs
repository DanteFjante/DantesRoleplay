...
internal sealed class SystemComponentTypeHandler
{
    ...
    private static Task<ToolEnvelope> Fail(IOperationLog log,PrivateOperatorAuthorizationDecision decision,string code,string why,string intent,string[]? procedures,string? fix=null)=>ToolRunner.RunAsync(log,"commit",intent,$"commit:{Kind}",procedures,()=>Task.FromResult(new ToolOutcome(null,$"Rejected {Kind}: {code}.",[fix??McpVerbCatalog.CommitCall(Kind,true)],new(code,why,fix??McpVerbCatalog.CommitCall(Kind,true)),GuardEvidenceJson:JsonSerializer.Serialize(decision.Evidence))),false);
