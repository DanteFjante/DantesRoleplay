...
public sealed class ProcedureHandler
{
    ...
    public async Task<ToolEnvelope> GetProcedureAsync(
        ...
        await ToolRunner.RunAsync(log, "get_procedure", async () =>
        {
            ...
            nextSteps.Add(
                $"{McpVerbCatalog.CommitCall("procedure", id, dryRun: true)} — only if following it revealed that it is wrong or incomplete.");
            ...
            if (!string.IsNullOrWhiteSpace(status))
            {
                if (!Enum.TryParse<ProcedureStatus>(status, ignoreCase: true, out var value))
                {
                    return ToolOutcome.Fail(
                        ...
                        $"{McpVerbCatalog.CommitCall("procedure", id)} — retry with status omitted, "
            ...
            if (dryRun)
            {
                return ToolOutcome.Ok(
                    ...
                    checks.Any(c => !c.Passed)
                        ...
                        : $"{McpVerbCatalog.CommitCall("procedure", id)} — the identical payload with dryRun omitted commits it.");
            }
