    ...
    IMechanicEngine engine) : IMechanicComposer
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var (resultKey, declaration) in requirements.Children.OrderBy(pair => pair.Key, StringComparer.Ordinal))
        {
            var invocations = BuildInvocations(resultKey, declaration, projection, ref ordinal);
        ...
        return new CompositionResult(projection with { Children = children });
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var run = await engine.RunAsync(
            ...
            cancellationToken);
        return run.Ok
            ? new(mechanic, composition.Projection, run)
        ...
        ref int ordinal)
    {
        ...
        if (string.IsNullOrWhiteSpace(declaration.ForEachContentsOf))
        {
            ...
            var input = ResolveInput(resultKey, declaration, projection.Input, itemId: null);
        ...
        foreach (var item in contents)
        {
            ...
            var input = ResolveInput(resultKey, declaration, projection.Input, item.Id);
            if (bindings.Error.Length > 0 || input.Error.Length > 0)
                return new([], bindings.Error.Length > 0 ? bindings.Error : input.Error);
