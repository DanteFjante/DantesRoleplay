...
///
/// A category is a dot-delimited path — <c>ruleset.dnd2024.gameplay.ability-checks.fixed-dc</c> —
...
///
/// Matching stops at a dot boundary: <c>ruleset.dnd2024.play</c> does not match
/// <c>ruleset.dnd2024.player</c>. Getting that wrong is the classic prefix-matching bug, and here
    ...
    public static bool TryValidate(string? path, out string problem)
    {
        ...
        if (string.IsNullOrWhiteSpace(path))
        {
            problem = "A category is required. Use a dot-delimited path, e.g. "
                + "\"ruleset.dnd2024.gameplay.ability-checks\" — or a single segment such as "
        ...
        if (!string.Equals(path, path.Trim(), StringComparison.Ordinal) || path.Any(char.IsWhiteSpace))
        {
            problem = $"'{path}' contains whitespace. A category path has no spaces — "
                + "use hyphens inside a segment, e.g. \"ruleset.dnd2024.ability-checks\".";
            return false;
        }
        ...
        foreach (var segment in segments)
        {
            if (segment.Length == 0)
            {
                problem = $"'{path}' has an empty segment — a leading, trailing or doubled dot. "
                    + "Every segment between dots must be a word, e.g. \"ruleset.dnd2024.combat\".";
                return false;
            }
