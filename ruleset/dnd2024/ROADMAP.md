...
- Character creation CC1, CC2A–CC2H2, CC3A–CC3C, CC3D1A, CC3E1–CC3E3, CC3F1, and the all-class basic-playable MVP are accepted. Pure role-bound JavaScript resolves Standard
  Array plus any of the four SRD background increases, plans any of nine species with canonical Size/base Speed, turns
  ...
  The MVP now composes any of four SRD backgrounds with any of twelve source-bound level-1 class
  models, applies fixed skills/tools/Common, optionally completes two Standard languages, the
  Soldier Gaming Set choice, and exact Bard/Monk class tool choices (including the complete
  37-tool SRD vocabulary), stores Monk/Rogue property-qualified Martial membership without
  claiming attack enforcement, makes Alert Initiative Proficiency an explicit level-derived opt-in
  while keeping Initiative Swap pending, and can resolve both legal cash equipment alternatives into one
  physical contained Gold Piece stack. It writes class-specific core state, an explicit pending
...
- Component convergence now has six accepted cohorts: Heroic Inspiration, the near-shape state
  group, normalized proficiencies, derived level/Armor Class, encounter lifecycle, and the item
  runtime core. Combat now
  uses explicit participation, locked Initiative, round, and turn entities with counted per-turn
  budgets and exact metric movement spending. The old encounter order snapshot, mutable cursor,
  participant budget, stored level, and final-AC authorities are retired. Runtime items now use a
  write-once definition link, positive quantity, and generic containment; the old item-instance and
  stack-key quantity owners are retired. Decomposed metric item definitions are the next ordered
  item/inventory sub-slice.
...
The minimum-playable chat boundary is defined in
[DND2024-MVP-CHAT-PLAYBOOK.md](DND2024-MVP-CHAT-PLAYBOOK.md); it intentionally uses the accepted
mechanics without waiting for source-complete character options.
