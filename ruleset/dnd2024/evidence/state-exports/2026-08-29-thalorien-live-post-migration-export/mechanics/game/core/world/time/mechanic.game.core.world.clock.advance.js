...
return {narration:world.name+' advances by '+ctx.input.minutes+' minutes.',effects:[{type:'component.set',entityId:world.id,definitionId:clockId,data:JSON.stringify(next)}],events:[{type:'game.core.world.clock.advanced',payload:{worldId:world.id,calendarId:clock.calendarId,beforeMinute:clock.currentMinute,afterMinute:next.currentMinute,beforeRevision:clock.revision,afterRevision:next.revision},entityIds:[world.id],scope:world.id}],data:{test:'world-clock-advance',worldId:world.id,minutes:ctx.input.minutes,previousMinute:clock.currentMinute,currentMinute:next.currentMinute,previousRevision:clock.revision,currentRevision:next.revision}};
