...
Status: **all-class basic-playable MVP and CC1–CC2H2 verified; no source-complete leaf active**
Ruleset alignment: `dnd2024-owned`
Source: `source.dnd2024.srd-5.2.1`, *Character Creation > Create Your Character* and
*Step 3: Ability Scores* (PDF pp. 19–21); *Character Origins > Character Backgrounds > Parts of a
Background / Soldier* (PDF p. 83); *Rules Glossary > Long Rest* (PDF p. 185) and
...
| Species/background/feat grants | D&D catalog JavaScript/content | missing | adoption Parent 11 gate requires family schemas and a transactional grant owner |
...
├─ background skills, tool, feat, languages, and equipment grants         [planned]
├─ class membership, saves, proficiencies, HP, and level-1 feature grants [partial: 12 models/core state verified]
├─ feat behavior for every supported origin path                          [planned]
├─ immutable selection/grant/completion receipts                          [planned]
...
| 3 | CC3 background/class/feat grant families | `gpt-5.6-sol` xhigh; subdivide by owner/grant family before implementation | CC1–CC2 and each target owner | One source-complete origin/class path has no unimplemented entitlement. |
...
are verified. CC2H3 cannot activate until an accepted non-Cantrip spell execution root exists.
...
schematized level-1 class models while leaving absent mechanics pending.
