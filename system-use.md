...
governs: query, commit, orient, any session operating this system
status: active
# Authored 2026-08-17 by Claude Fable 5 alongside VERB_MIGRATION.md. Move into Bootstrap/ in
# batch B4 of that migration — seeding it before the verbs exist would advertise calls that fail.
---
...
3. When you do not know a payload shape or which parameters a kind reads, call
   `query(kind: "capabilities")`. It is the exact catalog. Never guess a kind or a shape.
4. Before any `commit`, find and read the contract governing it:
   ...
   that against the commit you are about to make. Then cite what you read in `proceduresUsed`
   and say what you are doing in `intent`, in your own words. The audit records both.
5. Change with `commit(kind: ..., payload: ...)`. The kinds are `procedure`, `component`,
   `effects`, `mechanic`, `action`. Where `dryRun` is supported, ALWAYS call with
   `dryRun: true` first and read every named check or problem that comes back; then commit the
   identical payload. A dry run you did not read is worse than none.
6. Treat every failure as an instruction: the `fix` field names the literal next call. Make that
   call rather than retrying blind or giving up.
7. After a commit, confirm: query back what you wrote, and quote the returned `operationId` when
   reporting what you did.

## Constraints
- `query` never changes state. `commit` is the only write path, and `commit(kind: "effects")`
  and `commit(kind: "action")` are the only ways world state changes.
