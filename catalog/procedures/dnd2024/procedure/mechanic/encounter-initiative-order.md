---
id: dnd2024.procedure.mechanic.encounter-initiative-order
...
status: active
createdBy: "llm"
changeNote: "Feature 5 Slice 2: the governing contract for the encounter Initiative-order snapshot and its composition parent. The near-duplicate warning against dnd2024.procedure.mechanic.initiative was reviewed and not followed: that contract owns the individual resolver, which explicitly creates no encounter state, and dnd2024.procedure.mechanic.ruleset requires exactly one component per contract, so merging them would authorize two components from one contract."
---
...
4. Derive every Initiative count by composing dnd2024.mechanic.initiative.roll as a declared child, once per contained participant. Never read Dexterity, never roll a D20, and never accept a supplied count, modifier or die result in this rule's input.
...
## Constraints
- The rule must declare exactly one role, the encounter, with includeContents true, and exactly one child declaration bound to dnd2024.mechanic.initiative.roll with subject bound to $item.
