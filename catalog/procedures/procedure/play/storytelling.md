...
9. The conversation host records Player and assistant text word for word. Every assistant response
   also returns the closed situation update and only the durable truths established by that exact
   response. Continue, replace, or complete the current situation deliberately so play can resume in
   another client. Copy participant, location, and truth-subject entity IDs only from trusted bound
   context or safe execution evidence; otherwise use null for a participant/location ID and no subject
   ID for a truth. A role word such as `player`, `character`, or `target` is never an entity ID.

## Constraints
- This procedure creates no query/commit surface, mechanics, events, subscriptions, automation,
  authorization policy, or player identity. The generic conversation host, not narration, owns the
  durable verbatim transcript, typed situation history, and response-established truth records.
