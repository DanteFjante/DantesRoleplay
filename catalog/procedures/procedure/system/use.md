...
2. Read with `query(kind: ...)`. The kinds are `capabilities`, `procedures`, `categories`, `world`,
   ...
   For intent-oriented discovery, use `system.feature-search` within one application. The host
   automatically applies the active application's deterministic extension resolution. Operator
   diagnostics may pass `includeShadowed: true` to `system.catalog.search`; the response then
   includes shadowed records and `resolutionDiagnostics`. AI and feature search never select an
   overlay or extension profile. Exact record and qualified-ID reads remain exact, including
   qualified shadowed IDs. Resolve an
