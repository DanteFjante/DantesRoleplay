...
## Description
Submit starts one bounded durable focused-worker assignment for the currently selected application and state space. The caller supplies only the exact procedure, instruction, result schema, prior task handles, and optional names and JSON Pointers for their structured results. The host selects the workflow profile, worker model and provider, tools, current grant, operation budget, and deadline. A current state-space `Execute` standing grant must authorize the exact selected procedure.

The closed input is `stateSpaceId`; `procedure` with `definitionId`, positive `revision`, and 64-character uppercase `contentFingerprint`; `instruction`; `resultSchema`; `dependencyHandles`; and optional `dependencyInputs`. The instruction is 1–8,000 characters. `resultSchema` is a JSON string containing a bounded closed JSON Schema and is at most 16,000 bytes after canonicalization. `dependencyHandles` contains at most 16 durable handles with distinct task IDs, each carrying `taskId` and `commandId`. Each `dependencyInputs` entry supplies a unique bounded `name`, one of those exact handles, and an empty or RFC 6901 `jsonPointer` over the dependency's validated result data. The host reads every selected result under fresh authority immediately before execution and places at most 16 KiB of canonical values in a separately labelled prerequisite section. With no mappings, every dependency result is included under its retained handle identity.
...
- Missing, failed, stale, unauthorized, malformed, or oversized prerequisite results stop execution before a provider request.
