...
return {narration:s.name+'\'s turn budget is '+(i.mode==='record'?'recorded.':'corrected.'),data:{mode:i.mode,action:next.action,bonusAction:next.bonusAction,reaction:next.reaction,freeInteraction:next.freeInteraction,movementRemainingFeet:next.movementRemainingFeet,previous:previous,sourceRef:next.sourceRef},effects:[{type:i.mode==='record'?'component.add':'component.set',entityId:s.id,definitionId:DEF,data:JSON.stringify(next)}]};
