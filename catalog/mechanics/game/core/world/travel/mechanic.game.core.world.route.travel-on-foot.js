...
return { narration: traveller.name + ' travels from ' + origin.name + ' to ' + destination.name + ' along ' + route.name + '.', effects: [{ type: 'containment.move', entityId: traveller.id, toEntityId: destination.id, slot: 'presence' }, { type: 'component.set', entityId: world.id, definitionId: clockId, data: JSON.stringify(nextClock) }], data: { test: 'route-travel-on-foot', travellerId: traveller.id, routeId: route.id, worldId: world.id, originId: origin.id, destinationId: destination.id, mode: routeState.mode, minutes: routeState.durationMinutes, previousMinute: clockState.currentMinute, currentMinute: nextClock.currentMinute, previousRevision: clockState.revision, currentRevision: nextClock.revision } };
