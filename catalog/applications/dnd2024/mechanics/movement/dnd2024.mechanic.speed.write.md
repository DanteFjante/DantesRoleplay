---
id: dnd2024.mechanic.speed.write
category: dnd2024.ruleset.core.data.speed
name: Record creature Speed
scope: dnd2024-srd-5.2.1
status: active
---

## Description

Records or corrects one complete canonical D&D 2024 base-Speed profile in the current keyed metric
movement component. It does not move the creature, refresh a turn, or apply a temporary change.

## Matches
record creature speed
correct creature speed
set creature speed

## Requirements
```json
{"roles":{"subject":{"components":["dnd2024.creature.movement"],"description":"The creature whose absent or valid existing movement state is recorded or corrected."}},"inputSchema":{"type":"object","additionalProperties":false,"required":["mode","walkFeet","burrowFeet","climbFeet","flyFeet","swimFeet"],"properties":{"mode":{"type":"string","enum":["record","correct"]},"walkFeet":{"type":"integer","minimum":5,"maximum":1000},"burrowFeet":{"type":"integer","minimum":0,"maximum":1000},"climbFeet":{"type":"integer","minimum":0,"maximum":1000},"flyFeet":{"type":"integer","minimum":0,"maximum":1000},"swimFeet":{"type":"integer","minimum":0,"maximum":1000}}}}
```

## Input and result

Pass exactly `mode` (`record` or `correct`) and `walkFeet`, `burrowFeet`, `climbFeet`, `flyFeet`,
and `swimFeet`. Walking Speed is a 5–1,000 multiple of five; other modes are 0–1,000 multiples of
five. The mechanic converts those values to the current metric component and proposes one add or
set effect.
