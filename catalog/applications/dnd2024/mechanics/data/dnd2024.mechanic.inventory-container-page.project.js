...
function classMembership(node){var components=node.components;if(!components||Object.prototype.hasOwnProperty.call(components,LINK)||!Object.prototype.hasOwnProperty.call(components,'dnd2024.character.class-membership')||node.contains!==undefined&&(!Array.isArray(node.contains)||node.contains.length))return false;try{var value=JSON.parse(components['dnd2024.character.class-membership']);return object(value)&&object(value.classRef)&&text(value.classRef.entityId,200)&&integer(value.level,1,20)&&(value.subclassRef===undefined||object(value.subclassRef)&&text(value.subclassRef.entityId,200));}catch(error){return false;}}
