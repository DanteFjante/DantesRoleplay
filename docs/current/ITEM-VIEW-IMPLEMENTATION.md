...
**Status:** Published and verified. The live D&D page is revision 49, backed by catalog activation 51 and the compatible `dnd2024-main` binding revision 27. Publication used a reopened, integrity-checked database/blob backup, an export and six-document additive catalog review, matching activation/rebind dry runs and commits, and immutable draft readback of all 32 HTML/asset files. The existing independently trusted signing key attests the runtime targets and served bundle. Page revision 48 and the previous runtime remain available for recovery.

Live browser checks open Ganji's knife and quarterstaff in Player and DM, exercise return/focus, Back/Forward, perspective switching, the wide abilities layout and map wheel/button behavior, and measure the tabs at 1600, 1100, 820, 390 and 320 pixels. A separate Actor host using the same reviewed artifacts passes its own signed verification; all six Player item payloads equal the same-observer GM preview. Current records leave both item identities unknown to Ganji: Player Details stays neutral and association lists report partial supporting data; DM shows authored properties and empty associations. No knowledge or recipes were invented. A fresh live Player entry issues exactly one read per tab, and cached return issues none. Expiry and reauthorization may require bounded refreshes.

Runtime rehearsal exposed a missing generic component mapping for linked activity membership and targets. The mapping resolver now uses the authorized source-set component enumeration, with regression cases for exact mappings and missing references. Full acceptance passed 1,919 .NET tests, 309 web tests, type checking, and a solution build with zero warnings/errors. Both signed live verifiers pass; 2,638 runtime entities, 4,832 components, 1,210 events and zero notifications remain byte-for-byte equal to the prepublication snapshot. The implementation commit contains the verification receipt and local recovery/evidence locations. Item execution, crafting actions and authored knowledge/content repair remain excluded.
