...
**Status:** Complete. **Finding coverage:** P01, P11, P12, O03, R04. **Outcome:** the active D&D catalog now registers exact version-one `dnd2024.object.campaign-summary` and `dnd2024.object.faction-directory-page` objects and exposes them through the existing read-model transport. Campaign returns only its selected root plus read-only party participation references. The DM-only Faction object pages exact incoming `game.core.world.faction.in-world` membership, batches faction components and one-hop member/site/territory/ally/opponent references, carries count/completeness and source-revision cursor evidence, and never enumerates unrelated entities, character inventories or knowledge records. The generic relationship declaration gains optional `direction: incoming`; omission preserves the established outgoing meaning and legacy fingerprints. Compound collection reads run inside one serializable SQLite snapshot and reuse immutable definition/state authority only within their safe scope.
...
The website's active initial branch now performs exactly three application-data reads (audience context, selected Campaign entity and registered Campaign summary), below the eight-read gate. World Factions performs no initial request and issues one object-page read only when the DM opens that section; continuation is explicit. The existing search, selection, responsive controls, keyboard focus, loaded/total count and completeness behavior now operate over appended pages. Deeper Campaign sections retain the prior assembly only as an explicit deferred compatibility path until their later object slices; the initial branch no longer calls the full entity/location/faction, knowledge, chronology, inventory or current-scene assemblers.

Verification used a disposable copy of the recovered 294,522,880-byte development database; the source database was not changed. Catalog import retained the newer live Tensor Sect record and supplied the other authored records. Preview/activation/rebinding succeeded, and the final HTTP comparison returned all 35 unique Caldris factions as 25 plus 10 with stable ordering, correct completeness/cursor transitions, member and territory references, no inventory/notebook fields, DM success and Player denial (`403 READ_MODEL_FORBIDDEN`). A direct initial Campaign load returned the selected title and two party references in three reads with zero entity-directory, knowledge, chronology or inventory calls. Focused SQL evidence proves a prepared Faction page stays at the declared 12-command ceiling inside one consistent snapshot. Catalog validation passes 600 records with the seven unchanged legacy warnings and no live-data access; the web suites pass 260 Node and 71 mounted tests, TypeScript checking and the production Vite build at 74,913 gzip bytes of initial JavaScript; the Release solution build succeeds with zero warnings/errors; the complete .NET suite passes 1,935/1,935; and the opt-in MCP protocol walk passes 8/8 with its two unchanged environment-dependent cases skipped.

### Slice 5 (SC05) — Knowledge and chronology migration

**Status:** Complete. **Finding coverage:** P02, P03, P11. **Outcome:** knowledge and chronology now use one generic, bounded component-selection/hydration surface instead of enumerating every entity and probing components individually. The generic C# kernel accepts only opaque type IDs declared by the activated application binding; no D&D component ID, status, relationship kind, visibility value, calendar field or rule formula was added to it. Knowledge retains its existing audience/participation owner, familiar/unknown disclosure, exact document revisions and mandatory second disclosure check inside one consistent read snapshot. Chronology retains its existing audience owner, calendar/visibility/world/subject validation, ordering, response shape and 500-entry ceiling. Shared graph and state-space reads are reused only inside the explicit read snapshot and are discarded outside it.
...
Verification: real SQLite actor-notebook coverage, including participation and the disclosure recheck, stays within 16 SQL commands; real SQLite chronology coverage, including subject hydration, stays within 12. Twenty measured samples per profile select the same 419 relevant records in one SQL command after doubling the unrelated population from 2,638 to 5,276 entities, with unchanged median allocation and no proportional latency increase; the selected median is more than 98% lower than the retained legacy full-scan workload and warm source-file reads remain zero. Authorization, familiar/unknown redaction, stale revision/recheck, chronology filtering/bounds and selected-hydration tests pass. The web suites pass 260 Node and 71 mounted tests, TypeScript checking and the production Vite build; the Release build succeeds with zero warnings/errors; the complete .NET suite passes 1,938/1,938; and the opt-in MCP protocol walk passes 8/8 with its two unchanged environment-dependent cases skipped. No catalog record, runtime database or public response contract changed.
...
**Status:** Complete. **Finding coverage:** R01, R04. **Outcome:** `dnd2024.mechanic.rest.begin` now receives three exact versioned application objects for its creature, active World/clock/membership collection and immutable rest policy. The sandbox receives frozen JSON values and opaque role identities only; it still has no CLR, query, save or broadcast capability. Rest eligibility, policy interpretation, timing, outcome construction and the `dnd2024.rest.started` proposal remain in catalog JavaScript. The generic C# host materializes only the declared object contracts, records exact component and complete relationship snapshots, translates the unchanged generic effect proposal and checks every observed revision inside the existing atomic effect transaction. Legacy mechanics continue through the established projection resolver and effect translator; the selected reducer no longer receives or reparses its duplicate raw ECS component/relationship projection.
...
SC07 registers the permanent object IDs `dnd2024.object.rest-begin-creature`, `dnd2024.object.rest-begin-world` and `dnd2024.object.rest-begin-policy`, each at version one with an exact reviewed fingerprint. The existing mechanic ID, action request, role names, input, two effects, semantic event, narration, data, seed and audit path are unchanged. A complete bounded membership collection supplies relationship-presence and absence evidence; partial collections fail closed. Object source revisions become host-only transaction expectations, while legacy relationship effects retain their established current-edge adapter.

Verification: 61 focused application-execution, registered-object, collection, catalog, typed-effect and rest-begin tests pass, including short/long differential outputs, invalid and ineligible inputs, duplicate/replay behavior, legacy-projection removal and late-participant rollback of both effects and the event. Catalog validation accepts 600 records with only seven unchanged legacy warnings and no live-data access. The complete .NET suite passes 1,952/1,952. The web suite passes 260 Node and 71 mounted tests under the bundled Node 24 runtime, TypeScript checking succeeds and the production Vite build remains within the 74,913-byte gzip initial-JavaScript budget. The solution builds with zero warnings and errors. No live database, public protocol surface or runtime identity changed, so the opt-in MCP protocol walk was not required.

### Slice 8 (SC08) — Targeted committed-change delivery

**Status:** Complete. **Finding coverage:** P01, P12. **Outcome:** the generic typed-effect transaction now resolves changed component, relationship and transitive projection dependencies against registered application objects and stages bounded `object-change/v1` evidence before the root commit. Each row has a monotonic cursor, exact application/state-space and object version, closed read perspectives and generic reason; it never stores an entity ID or source component ID. Object changes within one operation coalesce, list membership follows declared relationship dependencies, unsupported entity lifecycle/containment mutations use an application-scoped recovery row, and a tracked unrelated mutation records only an internal no-dependency cursor marker. Evidence retention is bounded to 10,000 rows and replay to 256 rows.
...
Application-scoped SSE subscriptions are authorized against the existing ambient audience and binding owners before streaming; an actor cannot elevate to DM or cross application/state-space boundaries. The feed replays committed rows after reconnect, advances silently past tracked unrelated or other-audience rows, emits scoped invalidation for retention/size gaps and writes outside the typed-effect boundary, and retains the existing page-activation event. The D&D browser client starts at cursor zero to close the bootstrap race, ignores duplicate/out-of-order or cross-boundary frames, targets its Campaign summary cache, and exposes other exact object changes only to their matching application consumers. D&D object identity handling remains in that D&D client; no application ID, gameplay vocabulary or rule branch entered the C# kernel. Field patches and the Campaign/Factions cache-owner decision remain SC09.

Verification: 10 focused durable-delivery, rollback, dependency, audience, replay, continuity, out-of-band and unrelated-change tests pass. The complete .NET suite passes 1,962/1,962. The web suite passes 262 Node and 71 mounted tests under bundled Node 24, including direct duplicate/out-of-order client coverage; TypeScript checking and the production Vite build pass at 75,228 gzip bytes of initial JavaScript. The Release solution build succeeds with zero warnings/errors, and the opt-in MCP protocol walk passes 8/8 with its two established environment-dependent cases skipped. Catalog validation was not run because no authored catalog record changed. The ignored live database was not modified; the tested migration is applied by normal host startup.

### Slice 9 (SC09) — Browser object/query state

**Status:** Complete. **Finding coverage:** P11, P12, O03, R03. **Outcome:** the D&D browser now has one `BrowserObjectQueryState` owner for the migrated Campaign summary and Faction directory queries. It retains the existing `ViewReadClient` lifecycle underneath: each query has an eight-scope/30-second in-memory ceiling, latest-request cancellation, validated response fingerprints and keys covering every scope that can change during the page lifetime. Campaign keys separate perspective and campaign inside the immutable ambient seat; Faction keys additionally bind application, state space, seat, perspective, campaign, World and cursor. Faction continuation requires the first page's exact source revision. Both registered adapters now fail closed on the standard read-model envelope and retain its state-space, resolution, output-schema, result and source-revision fingerprints; those values are no longer discarded during Campaign projection. Exact SC08 notices retire only the matching migrated cache, Campaign changes also retire dependent character reads, unrelated object notices do not trigger the hub-wide stale notice, and continuity recovery still clears every private query scope.
...
The RTK pilot used Redux Toolkit 2.12.0's query core and store with a minimal fake query beside an equivalent `ViewReadClient` entry. Production-minified Vite output measured 117,032 raw / 33,410 gzip bytes for RTK versus 3,024 raw / 1,290 gzip bytes for the retained client. RTK's time-based unused-data expiry also did not replace the existing hard scope-count ceiling without another wrapper. Redux Toolkit and React Redux were therefore removed after the pilot: shipping both would duplicate cache ownership and add a second browser state runtime without improving this boundary. The selection/Campaign-detail flags now use a small Redux-style local reducer. Its edit lifecycle keeps drafts `editing`, `pending` or `failed`, never merges a draft into authoritative game data, and retires a pending draft only after an explicit server confirmation. No game-specific rule state or browser-library dependency was added to C#; the deeper non-object Campaign compatibility read remains uncached and outside this migrated owner.

The build gate still enforces the 90,000-byte initial ceiling and now follows the mandatory Campaign loader, connected projector and hub chunks separately. Acceptance measured 77,011 gzip bytes initially, 42,158 bytes of mandatory lazy JavaScript, 119,169 bytes through the first ready view and 188,764 bytes across all feature chunks. The browser now emits one `dnd2024.first-ready-view` navigation-to-active-view performance measure in addition to its stable marks, so the existing browser sampler can retain actual first-ready latency without inferring it from bundle time.

Verification: the web gate passes 264 Node and 78 mounted tests, including in-flight DM/Player replacement, denied-view last-good retention, incompatible and source-stale Faction pages, exact notification targeting and refetch, reconnect invalidation, hard count/age eviction and failed/pending/confirmed edit transitions. TypeScript and the production Vite build pass. The complete .NET suite passes 1,962/1,962, and the Release solution build succeeds with zero warnings/errors. Catalog validation and the MCP protocol walk were not run because no catalog record, protocol surface or dependency registration changed. The ignored live database was not modified.

### Slice 10 (SC10) — Character and Item migration

**Status:** Complete. **Finding coverage:** P01, R02, R03, R04. **Outcome:** three read-only `application-object/v1` contracts now establish the Character and Item structural migration boundary. `dnd2024.object.character-dossier-records` pins the exact creation, origin and feature-entitlement records used by the dossier. `dnd2024.object.inventory-item-instance-records` pins definition-link, quantity and equipment state and composes the exact `dnd2024.object.inventory-item-definition-records` reference, whose bounded sources include definition and authored activity records. Their version/fingerprint/reference graph is covered by activation tests. The published character dossier and Item Details/Recipes/Uses query IDs, mechanic projections and generated validators remain the compatibility surface while the read assembly migrates incrementally; no gameplay write owner or D&D rule moved into C#.
...
The three Item clients now use one typed, contract-bound transport adapter for the actor-scoped URL, status mapping, exact nine-key envelope, scope/query/schema binding, fingerprints, 65,536-byte data limit and source-revision comparison. Details retains its feature-owned media URL check; Recipes retains both independent `makes` and `uses` groups and their advancing offsets; Uses retains its own advancing page; all feature validators still bind observer, item and perspective. The hard eight-scope/30-second `ViewReadClient` lifecycle and existing request counts are unchanged. The production tree no longer contains the 1,958-line static hub source, 651-line fixture envelope or fixture audience policy: those modules and every consumer now live under `test/support`, removing their loader branches from production ownership without deleting parity coverage.

Verification: catalog validation passes for 600 records with the seven unchanged legacy Trail Survival warnings and confirms that no live data was touched. The web gate passes 264 Node and 78 mounted tests, including character abilities/layout, nested inventory navigation, both recipe groups, observer/perspective isolation, media rejection, forged envelopes, byte bounds, independent continuations, stale source revisions, cancellation and cache expiry. TypeScript and the production Vite build pass; all-feature JavaScript fell from 188,764 to 187,899 gzip bytes, while initial JavaScript is 77,010 bytes and the first-ready total is 119,166 bytes. The Release solution build succeeds with zero warnings/errors, the complete .NET suite passes 1,963/1,963, and the opt-in MCP protocol walk passes 8/8 with its two unchanged environment-dependent cases skipped. The ignored live database was not modified.

### Slice 11 (SC11) — Source and documentation hygiene

**Status:** Complete. **Finding coverage:** O01, O02, O03, O06, R02, R04. **Outcome:** the current tree no longer tracks the five reviewed alternate build directories: 678 generated files and 273,681,223 bytes were removed from this revision, with exact root-anchored ignore rules for each directory. A detached clean checkout contained none of those paths, restored and built the full solution with zero warnings/errors, and installed and built the web page with the declared Node 24 runtime. This changes the current revision only; Git history was not rewritten. The removed outputs remain recoverable from earlier commits.
...
The O02 inventory is closed file by file. `DevelopmentKnowledgeAudience.cs` and `DevelopmentKnowledgeAudienceTests.cs` were replaced by `LocalKnowledgeAudience.cs`, `LocalKnowledgeAudienceTests` and `LocalKnowledgeSeatLifecycleTests`. The uncompiled `KnowledgeBackgroundWorker.cs` has no remaining queue or processor type and current derived retrieval lifecycle is covered by `InteractionFeatureRetrievalTests`; the uncompiled `StoryPlanWorker.cs` has no remaining wake-queue type and current bounded planning is covered by `InteractionPlanningTests` plus the protocol walk's explicit removed-story-plan assertion. `CatalogWorldFeature7Tests.cs` is superseded by `ItemRecipesProjectionTests` and `ItemRecipeAssociationTests`; `CatalogWorldFeature9Tests.cs` by `CatalogWorldFeature18Tests`, `CatalogWorldMapVisualTests` and the web live-map placement tests. `CategoryToolTests.cs`, `InformationTests.cs` and `VerbToolTests.cs` targeted removed MCP signatures and are replaced at the closed public surface by `SystemCatalogProtocolTests` and `GuardTests`. `ProcedureBoundActionVerifierTests.cs` targeted removed proposal/verifier types and is replaced by the malformed-output and common-verifier cases in `InteractionPlanningTests`. `SqliteVecExtensionProbeTests.cs` targeted removed knowledge-vector types and is replaced by the disposable/stale/broken derived-index cases in `InteractionFeatureRetrievalTests`. `SessionFeature4Tests.cs` and `SnapshotFeature1Tests.cs` targeted withdrawn campaign-session/snapshot types; no supported production type or replacement behavior remains, so no new compatibility contract is claimed for them.

`SubscriptionStoreTests.cs` was the one excluded test owner still matching current production contracts. Seven registration, canonicalization, revision, mechanic-mode, payload-role and fanout checks are restored to the normal suite; only its removed public-commit-verb assertion was retired because public protocol ownership now belongs to `SystemCatalogProtocolTests`. `DantesRoleplay.Tests.csproj` now excludes only `ProtocolWalkTests.cs` during ordinary runs, and that established end-to-end walk remains available through `IncludeProtocolWalkTests=true`.

The remaining R02 fixture projection was already relocated in SC10. Thirteen map files referenced only by that fixture, 3,220,812 bytes in total, now live under `test/support/assets`; the production import graph has no path into test support. Other public and historical media remain untouched for the O04 ownership review in SC13. The two O03 components and their matching CSS have no remaining source, test or HTML consumer and were removed. Both HTML entry fallbacks now use the maintained `BootstrapShell` classes instead of preserving a second unavailable/connected presentation. The current entry guide no longer caches stale test totals or catalog counts; it routes contributors to checkout-relative verification and the dated slice evidence.

Verification: `SubscriptionStoreTests` pass 7/7. The web gate passes TypeScript, 264 Node tests, 78 mounted tests and the production Vite build; JavaScript remains 77,010 gzip bytes initially, 42,156 bytes of mandatory lazy code, 119,166 bytes through first-ready view and 187,899 bytes across all feature chunks. The Release solution build succeeds with zero warnings/errors and the complete .NET suite passes 1,970/1,970. The opt-in MCP protocol walk passes 8/8 with its two unchanged environment-dependent cases skipped. A separate clean checkout started at zero status entries and zero tracked alternate-output files, restored and built the Release solution, then completed a clean `npm ci` and production web build under Node 24. Catalog validation was not run because no catalog record changed. The ignored live database, authored catalog, exports and `_to_delete/` were not modified.
...
**Status:** Complete. **Finding coverage:** R01, remaining O02. **Outcome:** all 86 compatibility identities received an exact, machine-checked disposition and remain retained; none met every existing retirement condition. The schema-v2 retention policy requires a dated retain-all review, exact live-export and recovery evidence, unique identity membership and at least one non-empty reason group for every record. The 44 directly referenced identities account for 51 live legacy components, eight legacy entity-component owners, 678 mechanic executions and 2,293 procedure read/citation references. Eighteen exact `procedure.mechanic.dnd2024.*` files are also documents in current D&D activation revision 54 through `procedures-core-v2`; nine overlap the direct-reference set. A further 24 identities are named indirectly by retained live versions or historical payloads.
...
The remaining nine zero-reference candidates are explicitly retained because the established catalog import owner never deletes: removing only the files would make the next live export recreate them, while direct SQLite deletion would bypass that preservation boundary. No source, identity, history or live row was therefore removed. The reviewed ignored export contains 603 records and all 5,786 operations; all 86 policy identities are present and repository verification classifies their catalog content as unchanged. The verified Slice 14 recovery database passes integrity checking and has the same 5,786 operations and 91 blobs; all 91 live/backup blob paths, bytes and SHA-256 hashes match. The policy records the exact database hashes and group memberships. Slice 11's fourteen former-host files remain closed by their recorded replacement dispositions, and the sole normal test exclusion remains the intentional opt-in `ProtocolWalkTests.cs`.

Verification: the focused compatibility/catalog set passes 12/12. Catalog validation accepts 600 records with only the seven unchanged Trail Survival capability-schema warnings and touches no live data. The Release solution builds with zero warnings/errors and the complete .NET suite passes 1,987/1,987. The preserved opt-in protocol walk passes 8/8 with its two unchanged environment-dependent cases skipped. `roleplay verify` reports 599 unchanged records plus only the two known live-edited Caldris items and three known database-only fixtures; no compatibility identity is drifted. No MCP/dependency, web asset, database or blob content changed, so no live synchronization or web run was required.
...
Acceptance found and closed four integration defects rather than recording fixture-only success. The registered Campaign bootstrap now resolves the exact active participation references into deduplicated actor identities for an authorized DM or bound Actor while a GM Player preview remains identity-only; its initial path is five application-data reads. The Campaign summary contract now declares its campaign-selection source, and the generic catalog serializer preserves that optional query field. Generic ECS lifecycle migration now supports a same-ID upgrade to the current registered version without changing identity, while retaining validation, revision and audit behavior. The recovered database's two stale `dnd2024.item.quantity` v1 values were backed up and upgraded to v2 through that owner. The launcher now permits the observed roughly 90-second cold verification window, the character sampler waits for the actual terminal character state, and release signing no longer has a circular module dependency. These are generic host/catalog or D&D catalog/browser responsibilities respectively; no application ID, rule term, formula or outcome branch was added to C#.

The final live cutover materialized 3,582 catalog winners with no problems at activation revision 55 and rebound `dnd2024-main` at state-space revision 29. All eight readiness owners report ready: database, application registration, catalog activation/materialization, extension resolution, 21 callable query contracts, web page release and audience binding. The live database has 72 migrations, 2,879 entities, 5,254 components, no queued scheduled work, `PRAGMA integrity_check = ok` and no foreign-key violations. A consistent 74,334,208-byte SQLite backup has SHA-256 `F04991640E1DAAF664A69616E748284949FBCF7224641CCB1BCC0B34589AA698`; all 91 blobs remain present. Repository verification reports 599 unchanged records, the two intentionally live-edited item records and the same three database-only fixtures. Catalog validation accepts all 600 authored records with only the seven unchanged Trail Survival warnings.

The signed release manifest covers the index and 32 assets and has fingerprint `C1429F28DC463308EAB5AB8F670E5774B4D698C853321D00FE9063FB5835C292`. Live verification matched every served byte, cache policy and readiness owner and called representative query/action contracts. Rollback moved page revision 54 (`071ED0409C0421752CEFBD94C7D59E5AB7B5D1F9AB890EC5376473DF6332175C`) to retained revision 51 and then restored revision 54 with the exact original hash. Actual browser inspection proved one Ganji party actor, the canonical Monk 1 dossier, abilities, combat values and two inventory items for GM/DM and Actor seats; the Actor seat cannot enable DM. GM Player preview disclosed no DM faction/context data. The maintained map/board release passed wheel, touch-scroll, browser-pinch, geometry, text alternative and focus checks at 1,440, 1,024, 820, 390 and 320 pixels. The recovered campaign has no authorized map base, places or tactical scene, so the served view honestly showed unavailable state and no fictitious live map interaction was claimed.

Twenty sequential cold/warm browser-loader pairs were collected for each audience profile. Against the SC00 medians, selected Campaign p50 fell from 10,095.1/9,941.2 ms to 1,017.75/948.0 ms for GM/DM (89.9%/90.5%), from 2,843.3/2,768.2 ms to 987.65/910.3 ms for GM Player preview (65.3%/67.1%), and from 4,175.7/4,145.7 ms to 1,067.4/987.7 ms for Actor (74.4%/76.2%). No run exceeded 33 data requests, versus the 200-request complete-workload ceiling; the initial selected Campaign path uses five reads and Factions remains one object read plus an optional binding read. Character p50 was 1,211.85/1,209.65 ms for GM/DM and 1,213.7/1,224.1 ms for Actor, with all 40 states ready in each profile. Character timing is deliberately excluded for GM Player preview because the safe identity-only projection has no actor dossier authority. The sampler blocked automatic conversation-creation writes, so transactional edit/reducer acceptance comes from disposable tests rather than a live mutation. Earlier frozen SQL gates remain Campaign/Factions at most 12, Knowledge at most 16, Chronology at most 12 and the equivalent workload at most 80; SC05/SC12 evidence retains one SQL statement for the 419-record doubled fixture, zero warm source reads, at most two additional SQL commands and at most 10% allocation/median growth when unrelated entities double.

Final source verification passes the Release build with zero warnings/errors, all 1,988 ordinary .NET tests, and the opt-in MCP protocol walk with 8 passing and its two unchanged environment-dependent cases skipped. Web verification passes TypeScript, 264 Node tests, 78 mounted tests and the production build under Node 24; the build measures 77,011 gzip bytes initially, 42,310 bytes of mandatory lazy JavaScript, 119,321 bytes through first ready view and 188,051 bytes across all feature chunks. Responsive map/board browser automation passes 10/10 cases. Startup/reload, audience reversal, failed and stale object reads, pending/failed/confirmed edits, reducer execution, targeted subscriptions, reconnect, pagination at 99/100/101 boundaries, Character/Item navigation, forged responses, rollback and accessibility all have passing focused or full-suite owners. No required architecture or performance gate remains deferred; retained compatibility and the listed live/catalog differences keep their prior explicit dispositions rather than being silently deleted.
