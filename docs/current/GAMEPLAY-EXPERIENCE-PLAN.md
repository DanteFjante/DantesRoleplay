...
Updated 2026-09-12. P0–P4 are implemented on master through `48461729`; P5 is running. The copied-state host is available on port 16217 for rehearsal. **The live site on port 6217 has not received this feature release yet.** Root owns Git integration and acceptance; `website_integration` is the sole deployment writer, currently authorized for copied-state rehearsal only.
...
- Final capture usability review found two automatic-hook gaps after manual-helper acceptance: Codex sends `hook_event_name`, while the helper expected `event_name`, and Stop hooks require valid JSON or empty stdout rather than plain receipt text. P4 is correcting both and must exercise an actual installed/trusted isolated hook before automatic capture is accepted. Memory management currently uses private-operator MCP; no website memory controls are claimed.
...
- Copied world preview caught nonexistent truth references and three duplicate clues before any world write. Corrected packet `3b7477ab` reuses the three existing Q01 clues, creates two distinct canonical-story clues, and contains eleven new relationships and seventeen new entities. Six package and three namespace checks passed.
- Production review execution is an additional P5 gate: review submission currently queues work, while the existing validation execution service has no production runner. A dedicated explicitly enabled background worker is being added to invoke that same service with its existing authority, lease, lifecycle, fencing and receipt owners, a host-selected smaller model and no tools. Do not bypass review or synthesize a provider receipt. Bounded real review calls are authorized for this release after checking the queued work and provider configuration.
...
Next: complete automatic-hook acceptance and copied-state publication/browser acceptance, then authorize the sole writer to take fresh live preservation and publish. Verify live DM/Player, overlays, galleries, secrets, manual retrieval and restart stability before marking P5 complete.
