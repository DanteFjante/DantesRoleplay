...
| P3 Starting area | Eight maps, three illustrations, fourteen child locations, five unrevealed clues and one complete future situation integrated | Typed import, location-gallery attachment and live projections |
...
| P5 Delivery | Combined build and website checks passed; full .NET suite running; matching release built and copied DB migrated successfully | Finish regression/protocol checks, copied cutover, guarded live publication and second restart |
...
- Illustrations have ordinary location-gallery payloads, merged by content hash with fresh version checks. The five clues and canonical DM-only secret have explicit world-root containment. The complete future situation, “Before the Fourteenth Stroke,” is not a played event. Its package ID is application-qualified; existing campaign/world entity IDs are unchanged.
...
- Full .NET regression is running. Four failures found so far are corrected: bootstrap manual memory-kind listing (`d6fd10e4`), two private-storage catalog-coverage classifications (`777ab2de`), and prepared-package namespace (`48461729`). Rerun those affected checks after the suite, along with the required protocol walk; do not repeat the complete suite without a new reason.
...
Next: finish combined regression and protocol checks, complete copied-state publication and browser acceptance, then authorize the sole writer to take fresh live preservation and publish. Verify live DM/Player, overlays, galleries, secrets, manual retrieval and restart stability before marking P5 complete.
