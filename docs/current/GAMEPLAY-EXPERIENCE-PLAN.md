...
- Capture user messages and final outer-assistant responses. Exclude operational commentary, hidden reasoning, credentials, approval internals, and tool dumps. Do not ingest unrelated Codex tasks or other personal conversations by default. A captured statement is evidence of a conversation, not proof that an in-world action occurred.
