...
Status: **planning active; canonical multi-plane placement verified**
...
| World History UI | authorized knowledge interpretation | presentation-only | existing History screen and tests; no dated chronology owner exists |
...
├─ C. Dedicated dated World chronology                                      [awaiting confirmation]
│  ├─ C1. W19 permanent chronology record/schema/procedure                  [exact proposal awaiting confirmation]
│  └─ C2. Audience-safe live projection and History consumer                [blocked by C1]
...
├─ E. Entity-owned portraits and illustrations                             [missing]
│  ├─ E1. Media variants, alt text, provenance, hashes, lifecycle/history   [awaiting schema/ID confirmation]
│  └─ E2. Approved bytes and fail-closed web registry                       [blocked by E1 and asset review]
...
| 4 | World chronology owner | exact permanent ID/schema confirmation | Dated audience-classified records validate and import without copying knowledge/event logs |
| 5 | Player-safe structured directories | authorized projection contract | Player bytes contain only authorized people, creatures, factions, links, and aggregate counts |
| 6 | General entity media owner | exact permanent ID/schema and asset provenance approval | Entity variants include alt/provenance/lifecycle evidence and rollback is explicit |
| 7 | NPC profile owner | exact permanent ID/schema confirmation | Reusable biography/background renders separately from motive and D&D statistics |

## Next dependency boundary

W18 Slice 1 is verified. Live inspection confirms Crownmere and Merrowgate currently have no direct
child Location records, so the next map leaf cannot invent City markers or permanent geography.
W19 Slice 1 now carries the exact chronology ID/schema proposal and remains awaiting confirmation.
The general-media and NPC-profile leaves likewise still require exact permanent-ID/schema
confirmation before their implementation documents can become active.

## Confirmation gates

The user's 2026-08-30 request confirms multi-plane anchor semantics and the six outcome branches.
Separate confirmation remains required for exact chronology, general-media, and NPC-profile IDs and
schemas; reviewed asset approval; live Thalorien mutations; public surface changes; and completed
feature acceptance.

## Planning receipt

- Runtime artifacts created: none.
- Existing map-visual authority was verified rather than duplicated.
- The first implementation slice stops before live data and web changes.
